...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    def shopping_cart(self, request, **kwargs):
        ...
        if request.method == 'DELETE':
            ...
            return Response({'detail': 'Удаление из корзины прошло успешно'}, status=status.HTTP_204_NO_CONTENT)
    ...
    def favorite(self, request, **kwargs):
        ...
        if request.method == 'DELETE':
            ...
            return Response({'detail': 'Удаление из избранного прошло успешно'}, status=status.HTTP_204_NO_CONTENT)
    ...
    def subscribe(self, request, **kwargs):
        ...
        if request.method == 'POST':
            ...
            if Follow.objects.filter(user=request.user, following=following).exists():
                return Response({'errors': 'Подписка уже активна'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if request.method == 'DELETE':
            ...
            return Response({'detail': 'Подписка успешно отменена'}, status=status.HTTP_204_NO_CONTENT)
